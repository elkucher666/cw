...
async function showCalendar(room_id) {
    ...
    // Список всех дат мероприятий
    let response = await fetch(url, options);
    let result = await response.text();
    let applications = JSON.parse(result);

    // Для каждого мероприятия отобржаем данные в таблице
    let events = applications.map(function(application) {
        let booking_date = new Date(application.booking_date);
    
        let day = booking_date.getDate();
        let month = booking_date.getMonth() + 1;
        let year = booking_date.getFullYear();
        let event_title = application.booking_start.slice(0, 5) + " - " + application.booking_end.slice(0, 5);
